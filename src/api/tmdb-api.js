...
export const getTV = (args) => {
  ...
  export const getActors = () => {
    return fetch(
      `https://api.themoviedb.org/3/discover/person?api_key=${process.env.REACT_APP_TMDB_KEY}`
    ).then((response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      }
  ...
  export const getActorImages = ({ queryKey }) => {
    ...
    const { id } = idPart;
    return fetch(
      `https://api.themoviedb.org/3/person/{person_id}/images?api_key=${process.env.REACT_APP_TMDB_KEY}&language=en-US`
    ).then( (response) => {
      if (!response.ok) {
        throw new Error(response.json().message);
      }
